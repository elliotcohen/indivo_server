...
@marsloader
def medicationscheduleitem_list(request, limit, offset, status, order_by='created_at', record=None, carenet=None):
  """For 1:1 mapping of URLs to views. Calls _medication_list"""
  return _medicationscheduleitem_list(request, limit, offset, status, order_by, record, carenet)


@marsloader
def carenet_medicationscheduleitem_list(request, limit, offset, status, order_by='created_at', record=None, carenet=None):
  """For 1:1 mapping of URLs to views. Calls _medication_list"""
  return _medicationscheduleitem_list(request, limit, offset, status, order_by, record, carenet)
...
def _medicationscheduleitem_list(request, limit, offset, status, order_by='created_at', record=None, carenet=None):
  ...
  medicationscheduleitems = carenet_filter(carenet, Document.objects.filter(type=type_obj, replaced_by=None, record=record, status=status).order_by(processed_order_by)) 

  return render_template('reports/medicationscheduleitems', 
